...
{
    public class BTLUnidadAdministrativa
    {
        ...
        //}
        public Etimplementacion GetConfigImp()
        {
            DataTable table = GetObjetosNegocio().ObtenerConsulta(ScriptUnidadAdministrativa.GetConfigImp());
            List<Etimplementacion> lsResultado = UtilTablas.ConvertirDataTableToList<Etimplementacion>(table);
            Etimplementacion implementacion = lsResultado[0];

            return implementacion;
        }
        public List<ECat_AplicacionesModulos> GetConfiguracionARC(Erusuariobuzon usuario)
        {
            DataTable table = GetObjetosNegocio().ObtenerConsulta(ScriptUnidadAdministrativa.GetConfiguracionARC(usuario));
            List<ECat_AplicacionesModulos> lsResultado = UtilTablas.dtToListConfigARC(table);
            return lsResultado;
        }
